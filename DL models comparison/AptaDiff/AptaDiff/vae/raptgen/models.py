...
def train(epochs, model, train_loader, test_loader, optimizer, loss_fn=None, device="cuda", n_print=100,
          model_str="model.mdl", save_dir=Path("./"), threshold=20, beta=1, beta_schedule=False, force_matching=False,
          force_epochs=20, logs=True, position=0):
...
def profile_hmm_loss_fn(input, recon_param, mu, logvar, debug=False, test=False, beta=1, force_matching=False, match_cost=5, mu_filter:Optional[int]=None, device='cpu'):
    ...
    if test:
        if mu_filter:
            wise_pair = nn.PairwiseDistance(p=2)
            origin = torch.Tensor(np.zeros(mu.shape)).to(device)
            distances = wise_pair(mu, origin)
            indexes = torch.nonzero(distances <= mu_filter).flatten()
            if len(indexes):
                filtered_input = input[indexes]
                filtered_mu = mu[indexes]
                filtered_logvar = logvar[indexes]
                filtered_recon_param = (recon_param[0][indexes], recon_param[1][indexes])

                filtered_phmmloss = profile_hmm_loss(
                    filtered_recon_param,
                    filtered_input,
                    force_matching=force_matching,
                    match_cost=match_cost
                )
                filtered_kld = kld_loss(filtered_mu, filtered_logvar)
                return phmmloss.item(), kld.item(), filtered_phmmloss.item(), filtered_kld.item(), len(indexes)
            else:
                return phmmloss.item(), kld.item(), 0.0, 0.0, len(indexes)
