...
class MultinomialDiffusion(torch.nn.Module):
    def __init__(self, num_classes, shape, denoise_fn, vae=None, timesteps=1000,
    ...
    def predict_start(self, log_x_t, t, z):
        x_t = log_onehot_to_index(log_x_t)

        out = self._denoise_fn(t, x_t, z)
    ...
    def q_posterior(self, log_x_start, log_x_t, t):
        ...
        return log_EV_xtmin_given_xt_given_xstart  # q(xt-1/xt, x0)

    def p_pred(self, log_x, t, z):
    ...
    @torch.no_grad()
    def p_sample(self, log_x, t, z):
        model_log_prob = self.p_pred(log_x=log_x, t=t, z=z)
        out = self.log_sample_categorical(model_log_prob)
        return out


    @torch.no_grad()
    def p_sample_for_eval(self, log_x, t, z):
        model_log_prob = self.p_pred(log_x=log_x, t=t, z=z)
        out = self.log_sample_categorical_for_eval(model_log_prob, 0, times=t[0], mu=z)
        return model_log_prob, out
    ...
    def log_sample_categorical_for_eval(self, logits, flag, times, mu=torch.Tensor([0])):
        if flag:
            b = logits.shape[0]
            uniform = torch.rand_like(logits[0])
            uniform = uniform.unsqueeze(0)
            uniform = uniform.repeat(b, 1, 1)
        else:
            uniform = []
            if times == 999:
                self.rand_respo={}
                for i in range(logits.shape[0]):
                    seed = int(torch.sum(mu[i]) * 100000000)
                    ind = str(seed)
                    # print(seed)
                    ...
                    for j in range(times+1):
                        # print(j)
                        uni = torch.rand_like(logits[i])
                        self.rand_respo[ind].append(uni)
                    uniform.append(uni)
                print(uni.shape)
            else:
                for i in range(logits.shape[0]):
                    seed = int(torch.sum(mu[i]) * 100000000)
                    ind = str(seed)
                    uni = self.rand_respo[ind][times]
                    uniform.append(uni)
            uniform = torch.stack(uniform, dim=0)
    ...
    def compute_Lt(self, log_x_start, log_x_t, t, z, detach_mean=False):
        log_true_prob = self.q_posterior(
            log_x_start=log_x_start, log_x_t=log_x_t, t=t)

        log_model_prob = self.p_pred(log_x=log_x_t, t=t, z=z)
    ...
    def _train_loss(self, x, z):
        ...
        if self.loss_type == 'vb_stochastic':
            ...
            log_sample = self.q_sample(log_x_start=log_x_start, t=t)

            kl = self.compute_Lt(log_x_start, log_sample, t, z)
    ...
    def log_prob(self, x, z):
        ...
        if self.training:
            ...
            t, pt = self.sample_time(b, device, 'importance')
            kl = self.compute_Lt(
                log_x_start, self.q_sample(log_x_start=log_x_start, t=t), t, z)
            kl_prior = self.kl_prior(log_x_start)
    ...
    def sample_chain(self, args, model, num_samples, xy, data_shape):
        b = num_samples
        device = self.log_alpha.device
        uniform_logits = torch.zeros(
            (b, self.num_classes) + self.shape, device=device)
        log_z = self.log_sample_categorical_for_eval(uniform_logits, 1, self.num_classes)
        zs = torch.zeros((self.num_timesteps, b) + self.shape).long()

        sample_mu = torch.FloatTensor(xy).to(device)
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            t = torch.full((b,), i, device=device, dtype=torch.long)
            model_log_prob, log_z = self.p_sample_for_eval(log_z, t, z)
            model_log_prob = model_log_prob.cpu().numpy()
            log_prob0 = model_log_prob[0]
            prob0 = np.exp(log_prob0)
    ...
    def reconstruction_chain(self, args, model, loader, b):
        device = self.log_alpha.device
        zs_list = []
        uniform_logits = torch.zeros(
            (b, self.num_classes) + self.shape, device=device)
        log_z_0 = self.log_sample_categorical_for_eval(uniform_logits, 1, self.num_classes)
        print("reconstruction starts.")
        for j, (data, length) in enumerate(loader):
            x = data[0].to(device)
            mu = data[1].to(device)
            ...
            for i in reversed(range(0, self.num_timesteps)):
                print(f'Chain timestep {i:4d}', end='\r')
                t = torch.full((b,), i, device=device, dtype=torch.long)
                model_log_prob, log_z = self.p_sample_for_eval(log_z[:b], t, z)

                zs[i] = log_onehot_to_index(log_z)  # (1000, 64, 30)
            zs_list.append(zs)
        return zs_list

    def get_recon_x(self, args, recon_params, data_shape):
        from experiment import one_hot_index
