...
def provide_ax(func):
    from diffusion_utils.visualization import get_ax
...
def write_profile_hmm_svg(a, e_m,
    ...
    import svgwrite
    from vae_utils import State, Transition
    if is_log_proba:
        a = np.exp(a)
        e_m = np.exp(e_m)
...
def draw_most_probable(a, e_m, ax, fig=None, header="phmm", save=True, force=False, drawer=None, draw_only_insertion=True):
    ...
    from vae_utils import State, Transition
    from vae_utils import ProfileHMMSampler
    if drawer is None:
...
@provide_ax
def draw_step_probable(drawer, seq, path, ax, fig=None):
    # drawer = SeqLogoDrawer()
    drawer.draw_logo(seq, ax=ax)
