...
@click.option("--fast/--slow", help="whether or not to use the polytope model for pHMM probability calculation.", is_flag = True, default=False)
@click.option("--embed-size", help="the dimension of the embedding space", type=int, default= 2)
def main(seqpath, epochs, threshold, cuda_id, use_cuda, save_dir, fwd, rev, min_count, multi, reg_epochs, fast, embed_size):
